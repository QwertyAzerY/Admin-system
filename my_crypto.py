...
def generate_shared_key(p_key: bytes, priv_key:bytes, random : bytes):
    ...
    y=cv.y_recover(p_key)
    k=int.from_bytes(priv_key)
    P=Point(p_key, y, cv)
    shared_p=cv.mul_point(k, P)
    k1=int.from_bytes(random)
    shared_p=cv.mul_point(k1, shared_p)
    return shared_p.x.to_bytes(length=64)
...
class BlockCipherAdapter:
    BLOCK_SIZE = 16  # 16 байт = 128 бит
    KEY_SIZE = 32
    ...
    def _encrypt_block(self, block: bytes) -> bytes:
        ...
        for i in range(16):
            try:
                xored.append(block[i]^self.enc[i])
            except IndexError:
    ...
    def _decrypt_block(self, block: bytes) -> bytes:
        xored=[]
        plaintext=self.C.decryption(block)
        for i in range(16):
            try:
    ...
    def encrypt(self, plaintext: bytes) -> bytes:
        """Шифрует произвольную длину"""
        padded = self._pad(plaintext)
        blocks = [padded[i:i+self.BLOCK_SIZE] for i in range(0, len(padded), self.BLOCK_SIZE)]
        encrypted = b''.join(self._encrypt_block(block) for block in blocks)
        return encrypted

    def decrypt(self, ciphertext: bytes) -> bytes:
        """Расшифровывает и удаляет padding"""
        ...
        decrypted = b''.join(self._decrypt_block(block) for block in blocks)
        return self._unpad(decrypted)



def bytes_ints(b : bytes):
    arr=[]
    for byte in b:
        arr.append(int(byte))
    return arr
...
if __name__=="__main__":
    ...
    print(msg)
    print(ciphertext)
    plaintext=bytes(cbc_mode.decrypt(ciphertext))
    print(plaintext)
